...
class Abfrage(WindowAbfrage, QtGui.QWidget):
    ...
    """
    def __init__(self, parent, abfrage_haeufigkeit, verzoegerung, verzoegerungRichtig, meintenSie,
    ...
    def SaveAndExit(self):
        #self,pBFortschritt, distance, meintenSie, verzoegerung, id_aktuell, richtige_anzeigen, richtung, labPunkte, vokIds, abfragenGesamt, lektion, vokabel_deutsch, vokabel_fremd, buch
        ...
        meinSpeicher = Speicher(self.pBFortschritt.value(), str(self.distance), str(self.meinten_sie), self.verzoegerung,
                                ...
                                self.abfragenGesamt, self.lektion, self.vokabel_deutsch, self.vokabel_fremd, self.buch,
    ...
    def weitere_vokabel(self):
        ...
        if self.id_aktuell < len(self.vokabel_ids):
            ...
            self.close()
            self.WindowAuwertung = Auswertung(self, self.labPunkte.text(), len(self.vokabel_ids), self.sonderlektion)
            self.WindowAuwertung.show()

    def lektionsid_to_vokid(self, vokIds, haeufigkeit):
        """
        zu lektions id gibts die Vokabelid
        """
        vokabelIds = list()

        for i in range(haeufigkeit):
            for voks in vokIds:
                vokabelIds.append(voks)
